...
**Nora guia (2026-09-24):** a fixed companion Nora (`.guia`, built by JS, `aria-hidden`) appears once the hero leaves the lower 60% of the screen and hides at the final CTA/footer (hand-off to the CTA Nora). Sections carry `data-guia` (bubble line), `data-lado` (`e`/`d`) and optional `data-gesto="acena"`; when a section crosses the middle of the viewport she jumps in an arc to that side (X ease-in-out on `.guia`, Y rise-decelerate/fall-accelerate with squash on `.guia-pulo`), faces the content (`.guia-vira` mirrors on the right), then speaks for 3.4s. While scrolling she does quick steps (`.corre`), leans and lags on a damped spring driven by scroll velocity (`.guia-mola`), pupils follow the scroll direction; click = small hop + repeat line. Phones/tablets (≤960px): always bottom-right above the sticky CTA, hops in place, no side travel. Not rendered under reduced motion. Bubble lines only state real facts.
